    ...
    <div class="content">
      ...
      <el-card class="box-card" v-for="(item, index) in detail">
        ...
        <div class="text item" v-if="item.type == 'radio'" v-for="optionItem in item.options">
          <el-radio v-model="item.radioValue" :label="optionItem.id" style="margin: 5px;">{{ optionItem.title
          }}</el-radio>
        </div>
        ...
        <!--填空题展示-->
        <el-input v-if="item.type == 'text'" type="textarea" :rows="item.row" v-model="item.textValue" resize="none">
        </el-input>
